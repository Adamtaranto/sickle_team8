...
def getSummaryFile(fastq_dict):
	...
	last_window = len(fastq_dict[fastq_dict.keys()[0]].quality_means)
	
	#loop over each window position in read
	for window_pos in range(1, last_window+1):
		mean_values = []
		
		#loop over each read in fastq_dict and pull out
		#the mean value at that window position, store
		for read in fastq_dict:
			mean_values.append(fastq_dict[read].quality_means[window_pos])
		
		#calculate mean and std at each position
		mean_at_pos = mean(mean_values)
		sd_at_pos = std(mean_values)
		
		#add window position, mean and std to master list
		master_result.append([str(window_pos), str(mean_at_pos), str(sd_at_pos)])
	
	#create summary file
	summaryfile = header
	for i in range(0, len(master_result)):
